...
public class UnitParser {
        ...
        public boolean matches(String l) {
            ...
            if (l.length()>1) {
                ...
                return (l.equals(name) 
                        || l.equals(abb) 
                        || l.equals(name+plural)
                        || (abb.length()>1 && l.equals(abb + plural)));
    ...
    public static double parse(String val) throws IllegalArgumentException {
        ...
        Matcher m = Parsing.NUMBER_PATTERN.matcher(val);
        String unit = "";
        
        if (!m.find()) { throw new IllegalArgumentException("Bad unit value syntax: " + val); }
        double value = Double.parseDouble(val.substring(m.start(), m.end()).trim());
